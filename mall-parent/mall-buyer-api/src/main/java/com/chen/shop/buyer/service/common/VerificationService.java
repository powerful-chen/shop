package com.chen.shop.buyer.service.common;

import com.chen.shop.buyer.service.VerificationSourceService;
import com.chen.shop.common.enums.BusinessCodeEnum;
import com.chen.shop.common.utils.SerializableStream;
import com.chen.shop.common.utils.SliderImageCut;
import com.chen.shop.common.utils.SliderImageUtil;
...
public class VerificationService {
    ...
    public Result<SliderImageCut> createVerification(Integer verificationCode, String uuid) {
        /**
         * 目的：为了获取滑块验证的图片，同时 需要有一个滑块，然后图片上抠出来一个和滑块一样的图片出来
         *       抠出来的这个图片，移动到对应的位置上之后，重新组合为一个完整图片
         * 1、uuid 先判断请求是否合法
         ...
         * 5、从 资源图片上 抠一个和滑块一模一样的图片出来
         * 6、记录相关的信息 x轴信息
         * 7、相关的信息 存入 redis中，便于后续验证 key为：prefix + verification + uuid   value为：x轴移动坐标
         */
        if (uuid == null) {
        ...
        List<VerificationSourceVO> verificationSources = verificationVO.getVerificationSources();
        List<VerificationSourceVO> verificationSlider = verificationVO.getVerificationSlider();

        Random random = new Random();
        //随机选择需要切的图下标
        int resourceNum = random.nextInt(verificationSources.size());

        //随机选择剪切的模板下标
        int sliderNum = random.nextInt(verificationSlider.size());

        VerificationSourceVO resource = verificationSources.get(resourceNum);
    ...
    public Result preCheck(Integer verificationCode, String uuid, Integer xPos) {
        ...
        if (result) {
            ...
            //滑块验证成功之后 随后的一段时间如果有操作，认为是验证通过的，不需要重复进行滑块验证
